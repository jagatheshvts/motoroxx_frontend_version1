...
export class LocalStoreService {
  ...
  // }
  get_dayilreport(startdate,enddate, status,flag) {
    var branchid = localStorage.getItem("loginbranchid");

    return this.http.get<any>(`${environment.apiUrl}/superadmin/superadminauth/get_datafor_dailyreport?startdate=${startdate}&enddate=${enddate}&status=${status}&branchid=${branchid}&flag=${flag}`)
      .pipe(map(responsedata => {
        return responsedata
      }))
  ...
  gstsegment_report(startdate,enddate, status,flag){
      var branchid = localStorage.getItem("loginbranchid");
      return this.http.get<any>(`${environment.apiUrl}/superadmin/superadminauth/get_datafor_dailyreport?startdate=${startdate}&enddate=${enddate}&status=${status}&branchid=${branchid}&flag=${flag}`)
      .pipe(map(responsedata=>{
  ...
  // public exportAsExcelFile_task(json: any[], excelFileName: string, gstvariable, invoicedataamount, taxableamount): void {
  //   // 1. Create a shallow copy of data and shift everything down by two rows
  //   const shiftedData = [{}, {}].concat(json); // Add 2 empty objects to make space for two rows
    
  //   const worksheet: XLSX.WorkSheet = XLSX.utils.json_to_sheet(shiftedData, { skipHeader: true });
  
  //   // 2. Define header based on gstvariable
  //   let header: string[];
  //   if (gstvariable !== "") {
  //     header = ["Created Date", "Invoice Date", "Invoice No", "Name", "Mobile", "GST", "Brand", "Model", "Vehicle Number", "Spare", "Qty", "Rate", "Amount", "TaxAmount", "GST%", "CGST9%", "SGST9%", "CGST14%", "SGST14%"];
  //   } else {
  //     header = ["Created Date", "Invoice Date", "Invoice No", "Name", "Mobile", "Brand", "Model", "Vehicle Number", "Spare", "Qty", "Rate", "Amount"];
  //   }
  
  //   // 3. Add header at row 3 (index 2)
  //   const headerRowIndex = 2;
  //   header.forEach((title, index) => {
  //     const cellAddress = XLSX.utils.encode_cell({ r: headerRowIndex, c: index });
  //     worksheet[cellAddress] = { t: "s", v: title };
  //   });
  
  //   // 4. Add summary rows before header
  //   worksheet['A1'] = { t: 's', v: `Overall Invoice Amount: ${invoicedataamount}` };
  //   worksheet['A2'] = { t: 's', v: `Taxable Amount: ${taxableamount}` };
  //   let seperated = ((invoicedataamount - taxableamount)/2).toFixed(2);
  //   worksheet['A3'] = { t: 's', v: `CGST: ${seperated}` };
  //   worksheet['A4'] = { t: 's', v: `SGST: ${seperated}` };
  
  //   // 5. Set range
  //   const totalRows = json.length + 3; // +2 for summary, +1 for header
  //   const totalCols = header.length;
  //   worksheet['!ref'] = XLSX.utils.encode_range({ s: { r: 0, c: 0 }, e: { r: totalRows - 1, c: totalCols - 1 } });
  
  //   // 6. Auto width calculation
  ...
  //   // 7. Generate and save the Excel file
  ...
  public exportAsExcelFile_task(json: any[], excelFileName: string, gstvariable, invoicedataamount, taxableamount): void {
    // 1. Create a shallow copy of data and shift everything down by 5 rows (4 for gap + 1 for header)
    const shiftedData = [{}, {}, {}, {}, {}].concat(json);
    ...
    // 4. Add summary rows at the top
    worksheet['A1'] = { t: 's', v: `Overall Invoice Amount: ${invoicedataamount}` };
    worksheet['A2'] = { t: 's', v: `Taxable Amount: ${taxableamount}` };
    let separated = ((invoicedataamount - taxableamount)/2).toFixed(2);
    worksheet['A3'] = { t: 's', v: `CGST: ${separated}` };
    worksheet['A4'] = { t: 's', v: `SGST: ${separated}` };
  
    // 5. Set range
    const totalRows = json.length + 5; // +4 for summary gap, +1 for header
    const totalCols = header.length;
    worksheet['!ref'] = XLSX.utils.encode_range({ s: { r: 0, c: 0 }, e: { r: totalRows - 1, c: totalCols - 1 } });
  ...
  // it is an orginal code for the excel changed because of hsn 
  // public exportAsExcelFile_task(json: any[], excelFileName: string,gstvariable,invoicedataamount,taxableamount): void {
  //   const worksheet: XLSX.WorkSheet = XLSX.utils.json_to_sheet(json);
  ...
  public esportestspareexcel(json: any[], excelFileName: string): void {
    ...
    // Customize the header row to include the "Spare" column
    const header = ["Created Date", "estimate Date", "Name", "Mobile","GST", "Brand", "Model", "Vehicle Number", "Spare", "Qty", "Rate", "Amount",];
    const range = XLSX.utils.decode_range(worksheet['!ref']);
    for (let C = range.s.c; C <= range.e.c; ++C) {
      const address = XLSX.utils.encode_cell({ r: 0, c: C });
      if (!worksheet[address]) continue;
  ...
  getsearchdata_forwebapp(searchTerm) {
    ...
    return this.http.get<any>(`${environment.apiUrl}/superadmin/superadminauth/getsearchdata_forwebapp?name=${searchTerm}&branchid=${branchid}`)
      .pipe(map(responsedata => {
        return responsedata;
      }))
  }
  ...
  getjobcarddata_forpdfalert(jobcarid ) {

    return this.http.post<any>(`${environment.apiUrl}/shopusers/estimate/getjocardbyid`, { jobcarid: jobcarid, }).pipe(map(responsedata => {
  ...
  updaterunningno_branch(branchid, branchdata, flag) {
    ...
    createbranch(companyid, createdbranch,buyingControl,assignStaffControl,inventorycustomer,genralservice,singleBrand,multiBrand,selectedBrand,model_override,branchlogo) {

    return this.http.post<any>(`${environment.apiUrl}/superadmin/superadminauth/createbranch`, { companyid: companyid, createdbranch: createdbranch,buyingControl:buyingControl,assignStaffControl:assignStaffControl,inventorycustomer:inventorycustomer,genralservice:genralservice,singleBrand:singleBrand,multiBrand:multiBrand,model_override:model_override,selectedBrand:selectedBrand,branchlogo:branchlogo })
      .pipe(map(responsedata => {
        return responsedata
      }))
  ...
  getbranchbybranchidid2(branchid){
    return this.http.get<any>(`${environment.apiUrl}/superadmin/superadminauth/getbranchbybranchidid?branchid=${branchid}`)
    .pipe(map(responsedata => {
  ...
  get_inventory_products(page,pageSize,search,sortField,sortOrder) {
    let branchid = localStorage.getItem("loginbranchid");
    console.log("this is branch id", branchid);
    let companyid = localStorage.getItem("logincompanyid")
    
   
    return this.http.get<any>(`${environment.apiUrl}/superadmin/superadminauth/get_inventory_products?branchId=${branchid}&page=${page}&pageSize=${pageSize}&search=${search}&sortField=${sortField}&sortOrder=${sortOrder}`).pipe(map(responsedata=>{
      return responsedata
    }))
...
// return
  ...
  update_receipt_inventory(supplier_receipt_id,receiptdata,receiptupdateindex){

    let branchid = localStorage.getItem("loginbranchid");
    let companyid = localStorage.getItem("logincompanyid")
    return this.http.post<any>(`${environment.apiUrl}/superadmin/superadminauth/update_receipt_inventory`, {supplier_receipt_id:supplier_receipt_id,receiptdata:receiptdata,receiptupdateindex:receiptupdateindex}).pipe(map((responsedata)=>{
      return responsedata.response
    }))
  ...
  postcustomer(jobcardobj,location){
    ...
    return this.http.post<any>(`${environment.apiUrl}/shopusers/shopprofile/addshopprofile`, {company_id:companyid,branch_id:branchid,shopuserid:ShopuserId,name:jobcardobj.name,mobile:jobcardobj.mobile,address:location,gstname:jobcardobj.gstname,gstnumber:jobcardobj.gstnumber,
  ...
  checkvehicles_byvehcileno(vh_no){

    let branchid = localStorage.getItem("loginbranchid");
    let companyid = localStorage.getItem("logincompanyid")
    let ShopuserId = localStorage.getItem('ShopuserId')
    return this.http.get<any>(`${environment.apiUrl}/shopusers/addcomplaints/get_shopusers_jobcard_vehcile?branch_ID=${branchid}&vehno=${vh_no}`)
      .pipe(map(responsedata => {
        return responsedata
      }))

  } 
  checkvehicles_byjobid(jobid,userid){

    let branchid = localStorage.getItem("loginbranchid");
    let companyid = localStorage.getItem("logincompanyid")
    let ShopuserId = localStorage.getItem('ShopuserId')
    return this.http.get<any>(`${environment.apiUrl}/shopusers/addcomplaints/get_shopusers_jobcard_byjobid?branch_ID=${branchid}&jobid=${jobid}&userid=${userid}`)
      .pipe(map(responsedata => {
        return responsedata
      }))
  ...
  addjobcard(vehicle_id,odometervalue,fuelAmount,genralservicecheck,selectedModelpackage,formattedComplaintsstringy,
    mobile,userid,remarks,gs_selected_option_index,gs_selected_amount,advisor,delivery, deliveryin){
      ...
      return this.http.post<any>(`${environment.apiUrl}/shopusers/addcomplaints/addcomplaints_forwebapp`, {company_id:companyid,branch_id:branchid,ShopuserId:ShopuserId,mobile:mobile,userid:userid,odometerreading:odometervalue,
        ...
        typeofservice:servicetype,shopusername:shopusername,vehiclestatus:vehiclestatusstringy,advisor:advisor,delivery: delivery, deliveryin: deliveryin
  ...
  getpayments2_by_index(jobcardid,index){

    return this.http.get<any>(`${environment.apiUrl}/shopusers/addcomplaints/getpayments2_by_index?jobcard_id=${jobcardid}&index=${index}`)
    .pipe(map(responsedata => {
      return responsedata
    }))
  ...
  getfullinvoice_payments(jobcardid){

    return this.http.get<any>(`${environment.apiUrl}/shopusers/addcomplaints/getfullinvoice_payments?jobcard_id=${jobcardid}`)
    .pipe(map(responsedata => {
      return responsedata
    }))
  ...
  postpayments_new_multiinvoice(jobcard_id,vehiclestatus,userid,vehicle_id,paymentobj,paymentreceived,mobile,balance_amount,isfinalpayment,multiinvoiceindex){
    ...
    return this.http.post<any>(`${environment.apiUrl}/shopusers/addcomplaints/postpayments_new_multiinvoice`, {company_id:companyid,branch_id:branchid,jobcard_id:jobcard_id,userid:userid,vehicle_id:vehicle_id,payment:paymentobj,
      total_payment_received:paymentreceived,mobile:mobile,balance_amount:balance_amount,shopusername:shopusername,shopuserid:ShopuserId,finalpay:isfinalpayment,index:multiinvoiceindex
  ...
  savetemplate(data: any) {
    ...
    return this.http.post<any>(`${environment.apiUrl}/superadmin/superadminauth/garagetemplates`, payload)
      .pipe(
        map((responseData) => {
          console.log("Backend template response:", responseData.response);
          return responseData.response;
        })
      );
  }
  ...
  gettemplate() {
    let companyid = localStorage.getItem("logincompanyid");
    let branchid = localStorage.getItem("loginbranchid");
    console.log("this is company id and branch id>>>>>>", companyid, branchid)
    return this.http.get<any>(`${environment.apiUrl}/superadmin/superadminauth/getgaragetemplates?branch_id=${branchid}`).pipe(map((responsedata) => {
      console.log("this is gt template function data>>>>>>>", responsedata.response);
      return responsedata.response
    }))
  ...
  shopappupdateqty_productinventory(spares_inventory,jobcard_id,index){

   let stringyfyspares = JSON.stringify(spares_inventory)


   return this.http.post<any>(`${environment.apiUrl}/superadmin/superadminauth/shopappupdateqty_productinventory`, {jobcard_id:jobcard_id,productdetails:stringyfyspares,index:index}).pipe(map((responsedata)=>{
    return responsedata.result
  }))

  }

  get_qty_inventory(productid){

    return this.http.get<any>(`${environment.apiUrl}/superadmin/superadminauth/get_qty_inventory?productid=${productid}`).pipe(map((responsedata) => {
      console.log("this is gt template function data>>>>>>>", responsedata.response);
      return responsedata.response
    }))
  ...
  updatemultiinvoice(jobCardId, invoicedata,multiinvoiceindex) {

    var branchid = localStorage.getItem("loginbranchid");

    return this.http.post<any>(`${environment.apiUrl}/shopusers/estimate/invoice_multiinvoice_forwebapp`, { branch_id: branchid, jobCardId: jobCardId, invoicedata: invoicedata,multiinvoiceindex:multiinvoiceindex }).pipe(map(responsedata => {
      return responsedata
    }))
  }
  ...
  getVehiclePhotos(jobcardId){

    var branchid = localStorage.getItem("loginbranchid");

    return this.http.get<any>(`${environment.apiUrl}/shopusers/addcomplaints/getvehcilephotos?branch_id=${branchid}&jobcard_id=${jobcardId}`).pipe(map((responsedata) => {
      console.log("this is gt template function data>>>>>>>", responsedata);
      return responsedata
    }))
  ...
  get_historicaldata(mobile){

    let branchid = localStorage.getItem("loginbranchid");
    let companyid = localStorage.getItem("logincompanyid")
    let ShopuserId = localStorage.getItem('ShopuserId')
    return this.http.get<any>(`${environment.apiUrl}/superadmin/superadminauth/get_historicaldata?branch_id=${branchid}&mobile=${mobile}`)
      .pipe(map(responsedata => {
        return responsedata
      }))
  ...
  updatejobcard_api(updatedobj) {

    var branchid = localStorage.getItem("loginbranchid");

    return this.http.post<any>(`${environment.apiUrl}/superadmin/superadminauth/updatejobcard_api`, { customer_update: updatedobj}).pipe(map(responsedata => {
      return responsedata
    }))
  }
  ...
  hsn_report(startdate, enddate, flag) {
    var branchid = localStorage.getItem("loginbranchid");

    return this.http.get<any>(`${environment.apiUrl}/superadmin/superadminauth/hsn_report?startdate=${startdate}&enddate=${enddate}&flag=${flag}&branchid=${branchid}`)
      .pipe(map(responsedata => {
        return responsedata
      }))
...
hsnSearch(StartDate,EndDate,hsnSearch){

    let flag = "range"

    var branchid = localStorage.getItem("loginbranchid");
    return this.http.get<any>(`${environment.apiUrl}/superadmin/superadminauth/hsn_report?branchid=${branchid}&startdate=${StartDate}&enddate=${EndDate}&flag=${flag}`).pipe(map(resposedata=>{
      return resposedata
    }))
  }


  inventory_report(startdate, enddate) {
    var branchid = localStorage.getItem("loginbranchid");

    return this.http.get<any>(`${environment.apiUrl}/superadmin/superadminauth/inventory_report?startdate=${startdate}&enddate=${enddate}&branchid=${branchid}`)
      .pipe(map(responsedata => {
        return responsedata
      }))
  ...
  getvehcilecolours() {
    ...
    return this.http.get<any>(`${environment.apiUrl}/superadmin/superadminauth/get_vehcilecolour`)
      .pipe(map(responsedata => {
        return responsedata
      }))
  ...
  updatevehicle_colour(addcolour) {

    var branchid = localStorage.getItem("loginbranchid");

    return this.http.post<any>(`${environment.apiUrl}/superadmin/superadminauth/addvehiclecolour`, { color: addcolour}).pipe(map(responsedata => {
      return responsedata
    }))
  }
  ...
  getcustomerdata() {
    ...
    return this.http.get<any>(`${environment.apiUrl}/superadmin/superadminauth/getdata?branch_id=${branchId}`).pipe(
      map(responseData => {
        console.log("this is the message data", responseData.response);
        return responseData.response;  // <-- Make sure to return this!
      })
    );
  }
  ...
  getmessagetemplatedata() {
    let companyid = localStorage.getItem("logincompanyid");
    const branchid = localStorage.getItem("loginbranchid");
    return this.http.get<any>(`${environment.apiUrl}/superadmin/superadminauth/getMessageTemplates?branch_id=${branchid}`)
      .pipe(map(responsedata => {
        console.log("this is the message data", responsedata.response)
        return responsedata.response || [];
      }))
  ...
  savemessagealerts(data: any) {
    ...
    const payload = {
      ...
      templates: Array.isArray(data.templates) ? data.templates.map(template => ({
        name: template.name,
        messagetocustomer: template.messagetocustomer,
        availableVariables: template.availableVariables
      })) : []
    };
  
    console.log("Payload sent to API:", payload);
    return this.http.post<any>(`${environment.apiUrl}/superadmin/superadminauth/message_alert`, payload);
  }
