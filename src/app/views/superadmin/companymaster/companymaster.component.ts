...
export class CompanymasterComponent implements OnInit {
  ...
  singleBrand = new FormControl(false); 
  multiBrand = new FormControl(false); 
  isSingleBrandChecked = false;
  model_override :boolean= false;
  branddata_full: any;
  branddata: any;
  ...
  onLogoSelected_branch(event, logo) {
    const file = event.target.files[0];
    const reader = new FileReader();
    reader.onload = () => {
      logo = reader.result;
      this.branchlogo = logo
    };
    reader.readAsDataURL(file);
  }
  ...
  createbranch() {
      ...
      this.LocalStoreService.createbranch(this.company_id, this.registrationForm.value,this.buyingControl.value,this.assignStaffControl.value,this.inventorycustomer.value,this.genralservice.value,this.singleBrand.value,this.multiBrand.value,this.selectedBrand, this.model_override,this.branchlogo).subscribe(data => {
  ...
  // check_seperateCompany() {
  //   console.log("gst clicked", this.registrationForm.value.seperatecompany)

  //   if (this.registrationForm.value.seperatecompany == false) {
  //     this.addgst = false
  //   } else {
  //     this.addgst = true
  //   }
  ...
  toggleBrand(brandType: string): void {
    if (brandType === 'single') {
      this.isSingleBrandChecked = true;
      this.singleBrand.setValue(true); 
      this.multiBrand.setValue(false);
      this.model_override = true; // Set model_override to true
    } else {
      this.isSingleBrandChecked = false;
      this.multiBrand.setValue(true); 
      this.singleBrand.setValue(false); 
      this.model_override = false; // Reset model_override to false

    }
    console.log('model_override:', this.model_override);
  } 
...
console.log("console log all brand data",this.branddata,);
    ...
    onBrandSelect(selectedBrand: string) {
      this.selectedBrand = selectedBrand
      console.log("Selected brand:", selectedBrand);
    }


  toggleBrandupdate(checkboxType: string, data: any) {

    if (checkboxType === 'multiBrand') {
        data.single_brand = !data.multi_brand;
        console.log("single brand",data.single_brand)
        console.log("Multi brand",data.multi_brand)

    } else if (checkboxType === 'singleBrand') {
        data.multi_brand = !data.single_brand;
        console.log("single brand",data.single_brand)
        console.log("Multi brand",data.multi_brand)
    }
     if(data.single_brand )
    {
      data.model_override = true;
      console.log("multi over ride is true",data.model_override)
    }
    else{
      data.model_override = false;

      console.log("mulri over ride false",!data.model_override)
    }
    
}

onBrandSelectupdate(selectedBrand: string) {
