...
export class ReceiptComponent implements OnInit,PipeTransform {
  ...
  addInputField(i, conform_mrp_popup) {
    ...
    this.inputDataArray.push(
      { brand: '',model:'',modelList: [], category_id: '', category: '', 'prodid': '', 'prodName': '', HSN: '', unitmeasurement: '0.00', MRP: '', qty: '', rate: '0.00', gst: '', cgst: '0.00', sgst: '0.00', igst: '0.00', total: '0.00' },
    );
    ...
    this.LocalStoreService.sup_transbyid(selectedItem._id).subscribe(data => {
      ...
      if (data.response.length > 0) {
        ...
        this.sup_paymentdata.transaction.forEach(transaction => {
          if (transaction.receipttransaction.totalreceipt_amount) {
            receipt += transaction.receipttransaction.totalreceipt_amount;
          }
          if (transaction.receipttransaction.payment) {
            payment += transaction.receipttransaction.payment;
          }

          if (transaction.receipttransaction.grand_total) {
            receipt += transaction.receipttransaction.grand_total;
          }
        });
  ...
  openparticularpopup(particularpage, data, alldata, index) {
    ...
    this.particularpopupdata = alldata[index].receipttransaction.joinedparticulars || alldata[index].receipttransaction.particulars

    this.particularpopupdata_for = alldata[index].receipttransaction.transaction_type
    this.particularpopupdata_transdate = alldata[index].receipttransaction.transaction_date
    this.particularpopupdata_transref = alldata[index].receipttransaction.particulars
    this.particularpopupdata_amount = alldata[index].receipttransaction.grand_total || alldata[index].receipttransaction.payment
  ...
  previousparticular() {
    ...
    if (this.forparticularpopup_alldata[this.forparticularpopup_index]) {
      this.particularpopupdata_for = this.forparticularpopup_alldata[this.forparticularpopup_index].receipttransaction.transaction_type
      this.particularpopupdata_transdate = this.forparticularpopup_alldata[this.forparticularpopup_index].receipttransaction.transaction_date
      this.particularpopupdata_transref = this.forparticularpopup_alldata[this.forparticularpopup_index].receipttransaction.particulars
      this.particularpopupdata_amount = this.forparticularpopup_alldata[this.forparticularpopup_index].receipttransaction.grand_total || this.forparticularpopup_alldata[this.forparticularpopup_index].receipttransaction.payment
      this.particularpopupdata = this.forparticularpopup_alldata[this.forparticularpopup_index].receipttransaction.joinedparticulars || this.forparticularpopup_alldata[this.forparticularpopup_index].receipttransaction.particulars
    }
  }
  ...
  editreceipt_popup(sup_paymentdata, i) {
    ...
    console.log("the receipt edit popuop",this.receiptdata_foredit)
  }

  updateBrand(data: any, selectedBrandId: any): void {


      const selectedBrandData = this.branddata_full[data];
      if (selectedBrandData) {
        this.seletedbrand_model = selectedBrandData.bikespackage.map((model)=>{
          model.bikename;
          return model;
        });
      } else {
  ...
  // whenbrandselect(selectedBrand,i){
  //     if (selectedBrand) {
  //       const selectedBrandData = this.branddata_full[selectedBrand];
  //       if (selectedBrandData) {
  //         this.seletedbrand_model = selectedBrandData.bikespackage
  //       } else {
  //         this.seletedbrand_model = []; 
  //       }
  //     }
  ...
  // changevaluemodel(i,model){
  //   this.inputDataArray[i].model = model
  // }


  whenbrandselect(selectedBrand, rowIndex) {
    if (selectedBrand) {
      const selectedBrandData = this.branddata_full[selectedBrand];
      if (selectedBrandData) {
        // Assign the specific models to the current row's `modelList`
        this.inputDataArray[rowIndex].modelList = selectedBrandData.bikespackage;
      } else {
        this.inputDataArray[rowIndex].modelList = [];
      }
      // Reset the model value for the current row
      this.inputDataArray[rowIndex].model = null;
    }
  }


  onValueChange(data: any, field: string) {
    // Ensure the value remains a valid number and round it to 2 decimal places
    if (data[field] !== null && data[field] !== undefined && !isNaN(data[field])) {
      data[field] = Math.round(parseFloat(data[field]) * 100) / 100; // Round to 2 decimal places
    }
  }
